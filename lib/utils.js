...
Object.defineProperty(utils, 'screenWidth', {
...
utils.backup = (original) => {
  ...
  try {
    if (fs.existsSync(original)) {
      ...
      let i = 1;
      for (; fs.existsSync(backup); i++) {
        const old = backup;
        const [, lastNumber] = backup.match(/-backup([0-9]+)$/) || [];
        if (lastNumber) {
          backup = backup.replace(
            `-backup${lastNumber}`,
            `-backup${Number(lastNumber) + 1}`
          );
        } else {
          backup += i;
        }
        console.log(`Backup exists: '${old}', trying '${backup}'`);
      }
      if (i > 1) {
        console.warn(
          'Too many previous backups exist, might wanna clean those up!'
        );
      }
    ...
    return (halt) => {
      ...
      try {
        ...
        if (halt) throw error;
        console.error(
          `Couldn't restore '${backup}' -> ${original}. ${error.message}`
        );
        return false;
      }
    };
...
utils.multiRestorer = () => {
  ...
  /** @type {restorer} */
  const restore = (...args) =>
    !restorers.map((r) => r(...args)).includes(false);
  const discard = (...args) =>
    !discarders.map((r) => r(...args)).includes(false);
  return { push, restore, discard };
};
...
utils.modifyFile = (
  filename,
  data,
  { silent, write = fs.writeFileSync, backup = true } = {}
) => {
  let _data;
  try {
...
utils.runCmd = (cmd, args, opts) => {
  opts = opts || {};
  const logMeta = [cmd, ...args, opts.cwd && `(in '${opts.cwd}')`].filter(
    Boolean
  );
 ...
 */
utils.getDependencyPackageJson = (...deps) =>
  deps.map((dep) => {
 ...
 */
utils.getDependencyNodeModulePath = (...deps) =>
  deps.map((dep) => Path.join(utils.cwd, 'node_modules', dep));
...
utils.isDependencyRelative = (dep) =>
  dep.match(/[/\\.]/) && !dep.startsWith('@');

// utils.unique = array => array.filter
utils.readDirs = (dir) =>
  fs.readdirSync(dir).filter((dir) => {
    if (dir === '.') return false;
    if (dir === '..') return false;
...
utils.pathDiff = (
  ...
  const short = (s) =>
    utils.shortString(s, { length: shortLength, join: shortJoin });
  ...
  function forward(a, b) {
    ...
    return [a_split.join(''), b_split.join(''), common.join('')];
  }
...
utils.shortString = (
  string,
  { length: maxLength = utils.screenWidth, ratio = 0.37, join = '…' } = {}
) => {
