...
async function copy(dep, argv) {
  ...
  console.log(`[${utils.getTime()}] Copied ${logRelPath(destinationDir, sourceDir)}`);
  ...
  watch(sourceDir, { recursive: true }, utils.debounce(adjustWatchChanges(s => onChange(s, { retries })), 100));
  ...
  async function onChange(source, { retries = 3 } = {}) {
    ...
    try {
      ...
      if (filter(source)) {
        if (await fs.pathExists(source)) {
          ...
          if (await utils.compareFiles(source, destination)) {
            console.log(`[${utils.getTime()}] Skipped ${logRelPath(destination, source)} (same content)`);
          } else {
            await fs.copy(source, destination, { filter });
            // log(`Updated ${destination} <- ${source}`);
            console.log(`[${utils.getTime()}] Updated ${logRelPath(destination, source)}`);
          }
        } else {
      ...
      if (retries && retries > 0) {
        ...
        console.error(`Couldn't copy file: '${utils.shortString(source)}'. ${error.message}`);
      }
    }
  }
}
